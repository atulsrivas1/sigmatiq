...
try:
    from sigma_core.registry.signals_registry import fetch_signals as db_fetch_signals
except Exception:
...
def model_performance(model_id: str, risk_profile: Optional[str] = Query(None), start: Optional[str] = Query(None), end: Optional[str] = Query(None)):
    ...
    try:
        ...
        try:
            from sigma_core.registry.backtest_registry import leaderboard as _db_leaderboard
