...
# Also attempt to add sibling edge-core if present (monorepo layout)
try:
    _EDGE_CORE_DIR = _PARENT / 'edge-core'
    if _EDGE_CORE_DIR.exists() and str(_EDGE_CORE_DIR) not in _sys.path:
        _sys.path.insert(0, str(_EDGE_CORE_DIR))
except Exception:
    pass
...
from edge_core.backtest.engine import run_backtest
from edge_core.features.builder import select_features as select_features_train
from edge_core.cv.splits import PurgedEmbargoedWalkForwardSplit
...
from edge_core.registry.backtest_registry import create_backtest_run, leaderboard as db_leaderboard, create_backtest_folds
try:
...
# Workspace is the repository root (Sigmatix-Edge)
ROOT_DIR = _Path(__file__).resolve().parent.parent
WS_DIR = ROOT_DIR

app = FastAPI(title="Sigmatiq Edge API")
