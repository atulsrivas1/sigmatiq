(function(){
  ...
  function render(root, schema){
    ...
    const hero = el('header', { class: 'pack-hero' }, [
      el('div', { class: 'inner' }, [
        el('div', {}, [ el('div', { class: 'small', text: 'Sigmatiq Edge' }), el('h1', { class: 'h3', text: 'UI Panel Renderer — Themed' }) ]),
        el('div', { class: 'row' }, [
          ...
          (() => {
            ...
            [['zero','ZeroEdge'],['swing','SwingEdge'],['long','LongEdge'],['overnight','OvernightEdge'],['custom','CustomizedEdge']].forEach(([k,v])=>{
              sel.appendChild(el('option', { value: k, text: v }));
            });
            sel.addEventListener('change', e => document.documentElement.setAttribute('data-edge', e.target.value));
            return sel;
          })()
        ])
      ])
    ...
    function draw(data){
      ...
      // line color = accent
      ctx.strokeStyle = getComputedStyle(document.documentElement).getPropertyValue('--edge-accent') || '#3B82F6';
      ctx.lineWidth = 2;
      ctx.beginPath();
