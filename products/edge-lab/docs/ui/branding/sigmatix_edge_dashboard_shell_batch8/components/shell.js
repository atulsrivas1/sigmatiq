  ...
  function initToggles(){
    ...
    if (packSel){
      packSel.addEventListener('change', e => {
        document.documentElement.setAttribute('data-edge', e.target.value);
        try { localStorage.setItem('sig_edge', e.target.value); } catch {}
      });
      try { const saved = localStorage.getItem('sig_edge'); if (saved) { document.documentElement.setAttribute('data-edge', saved); packSel.value = saved; } } catch {}
    }
    if (menuBtn){
